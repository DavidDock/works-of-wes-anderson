...
Each film has one image, it's poster. More unique posters were chosen to add interest, the posters are from [The-Poster-DB](https://theposterdb.com/).
...
- Makesure the development environment has:
  ...
  - Your settings.py is set up to use DATABASE_URL(ElephantSQL used for this project's database), CLOUDINARY_URL  (Used for hosting images uploaded) and SECRET_KEY (Django secret key), all pointing to your env.py where the environment variables are stored or will be found in Heroku convig vars when deployed
...
- Add following convig Vars to new App:
  - DATABASE_URL with database value in env.py (Copied from relevant ElephantSQL database instance)
...
- Add Heroku url to Allowed hosts in settings.py file
